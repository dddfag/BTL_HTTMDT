...
export const CheckoutPage = ({ setIsCartSectionActive }) => {
  ...
  const placeOrderFn = async (e) => {
    ...
    try {
      ...
      // Fetch updated user data after successful order placement
      const updatedUserData = await axios.get(`${serverUrl}api/v1/auth/getUserData?email=${email}`);
      dispatch(getUserData(updatedUserData.data));
      localStorage.setItem("UserData", JSON.stringify({ ...updatedUserData.data, loginToken: JSON.parse(localStorage.getItem("UserData"))?.loginToken }));
